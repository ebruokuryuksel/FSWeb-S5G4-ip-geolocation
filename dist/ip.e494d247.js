...
function cardInfo(info) {
  ...
  card.append(ipAdress, country, latLong, city, time, money, isp);
  ipAdress.textContent = info.sorgu;
  country.textContent = "".concat(info.ülke, " ").concat(info.ülkeKodu);
  latLong.textContent = "Enlem : ".concat(info.enlem, " Boylam:").concat(info.boylam);
  city.textContent = "\u015Eehir : ".concat(info.şehir);
  time.textContent = "Saat dilimi : ".concat(info.saatdilimi);
  money.textContent = "Para birimi : ".concat(info.parabirimi);
  isp.textContent = "ISP : ".concat(info.isp);
  return newCard;
}
;
